package com.tre.centralkitchen.common.mapper;
...
public interface BaseMapperPlus<M, T, V> extends BaseMapper<T> {
    ...
    /**
     * Each record is updated when it exists
     *
     * @param entityList           data list
     * @param queryWrapperFunction query function
     * @return success or fail
     */
    default boolean updateByQueryWrapper(Collection<T> entityList, Function<T, LambdaUpdateWrapper<T>> queryWrapperFunction) {
        return SqlHelper.executeBatch(this.currentModelClass(), log, entityList, DEFAULT_BATCH_SIZE,
                (sqlSession, entity) -> {
                    Map<String, Object> param = new HashMap<>();
                    param.put(Constants.ENTITY, entity);
                    param.put(Constants.WRAPPER, queryWrapperFunction.apply(entity));
                    sqlSession.update(SqlHelper.getSqlStatement(this.currentMapperClass(), SqlMethod.UPDATE), param);
                });
    }

    /**
     * Each record is updated when it exists, and inserted when it does not exist
     *
     * @param entityList           data list
     * @param queryWrapperFunction query function
     * @return success or fail
     */
    default boolean insertOrUpdateByQueryWrapper(Collection<T> entityList, Function<T, LambdaUpdateWrapper<T>> queryWrapperFunction) {
        return SqlHelper.saveOrUpdateBatch(this.currentModelClass(), this.currentMapperClass(), this.log, entityList, DEFAULT_BATCH_SIZE,
                (sqlSession, entity) -> !this.exists(queryWrapperFunction.apply(entity)),
                (sqlSession, entity) -> {
                    Map<String, Object> param = new HashMap<>();
                    param.put(Constants.ENTITY, entity);
                    param.put(Constants.WRAPPER, queryWrapperFunction.apply(entity));
                    String sqlStatement = SqlHelper.getSqlStatement(this.currentMapperClass(), SqlMethod.UPDATE);
                    sqlSession.update(sqlStatement, param);
                });
    }
