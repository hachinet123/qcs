...
import com.tre.centralkitchen.domain.vo.system.FixAmountResultConfirmVo;
import com.tre.centralkitchen.mapper.MtMailcontrolMapper;
import com.tre.centralkitchen.mapper.TrOutTransbillMapper;
import com.tre.centralkitchen.mapper.TrProducedMapper;
...
@Service
public class FixAmountResultConfirmServiceImpl implements FixAmountResultConfirmService {
    ...
    public void fixAmountResultConfirm ( List<FixAmountResultConfirmBo> bos, String userId ) {
        for (FixAmountResultConfirmBo bo : bos) {
            decideActualFixAmountProducts(bo, userId);
        }
    }

    @Override
    public TableDataInfo search ( FixAmountResultConfirmSearchBo bo, PageQuery pageQuery ) {
        ...
        return TableDataInfo.build(controlMapper.fixAmountResultConfirmVoPage(pageQuery.build(), bo.getCenterId(), mailNos));
    }

    @Override
    public void downloadCSV ( FixAmountResultConfirmSearchBo bo, HttpServletResponse response ) {
        List<Short> mailNos = new ArrayList<>();
        if (!bo.getMailNo().isBlank()) {
            mailNos = Arrays.stream(bo.getMailNo().split(StringConstants.COMMA)).distinct().map(Short::parseShort).collect(Collectors.toList());
        }
        List<FixAmountResultConfirmVo> records = controlMapper.fixAmountResultConfirmVoPage(bo.getCenterId(), mailNos);
        SimpleCsvTableUtils.easyCsvExport(response, MailConstants.FIX_AMOUNT_RESULT_CONFIRM_CSV_NAME, records, FixAmountResultConfirmVo.class);
    }

    private void decideActualFixAmountProducts ( FixAmountResultConfirmBo bo, String userId ) {

        producedMapper.decideActualFixAmountProducts(bo.getCenterId(), bo.getMailNo(), userId, bo.getInsFuncId(), bo.getInsOpeId());

        List<DecideProductionResultsVo> decideProductionResultsVos = trOutTransbillMapper.decideProductionResultsTemp(bo.getCenterId(), bo.getMailNo(), 2);
        Collections.sort(decideProductionResultsVos, Comparator.comparingInt(DecideProductionResultsVo::getMailNo)
                .thenComparingInt(DecideProductionResultsVo::getStoreId)
                .thenComparing(DecideProductionResultsVo::getDlvschedDate));
        ...
        trOutTransbillMapper.decideProductionResultsOnFixAmount(bo.getCenterId(), bo.getMailNo(), userId, bo.getInsFuncId(), bo.getInsOpeId(), uuid.toString());
        trOutTransbillMapper.decideProductionResultsPacks(bo.getCenterId(), bo.getMailNo(), 2, userId, bo.getInsFuncId(), bo.getInsOpeId(), uuid.toString());
        if(!pos.isEmpty()) {
